                        ...
                        if os.path.isfile(os.path.join(csv_path, csv_file)):
                            ...
                            for cluster_label, images in cluster_images_padded.items():
                                if images:
                                    ...
                                    # Create an empty image with the maximum height and width required
                                    empty_image = np.zeros((max_height, x1 + horizontal_concatenation.shape[1], 4), np.uint8)
                                    empty_image[:, :, 3] = 255
                                    
                                    # Overlay the horizontally concatenated images onto the empty image at the specified coordinates
                                    empty_image[:, x1:x1 + horizontal_concatenation.shape[1]] = horizontal_concatenation


                                    # Save the resulting image to a file
                                    resulting_image_path = os.path.join(ID_path, f"{s}_{g}_{tr}_{ti}_Cluster_{cluster_label}_PCA_UMAP_Concatenated.tif")
                                    cv2.imwrite(resulting_image_path, empty_image)
