...
csv_file = f"Morphology_UMAP_HDBSCAN_{n_neighbors}.csv"
data = os.path.join(csv_path, csv_file)

# Read the CSV file
df = pd.read_csv(data)

# Define categories for iteration
subject = ["R1", "R2"]
group = ["CNEURO1", "VEH"]
treatment = ["ESC", "SS"]
        ...
        for tr in treatment:
            for ti in tissue:
                ...
                if combination_exists:
                    ...
                    output_file = f"{s}_{g}_{tr}_{ti}_Morphology_UMAP_HDBSCAN_{n_neighbors}.csv"
...
for s in subject:
    for g in group:
        for tr in treatment:
            for ti in tissue:
                csv_file_name = f"{s}_{g}_{tr}_{ti}_Morphology_UMAP_HDBSCAN_{n_neighbors}.csv"
